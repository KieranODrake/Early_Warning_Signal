#' KD 26 June 2023 - Adapted from 'wave define - hosp - growth - overlay.R'
#' Simplified overlay
...
#' gam_fitting(), growth_method() in wave_define_growth_model_200_wave_col.R (this is different from the function used for the hospitalisation data because only 100 columns were used for dates whereas some GAMs generated more than 100 waves start dates)
#' 
#' ###############################
...
folder <- 'C:/Users/kdrake/OneDrive - Imperial College London/Documents/GitHub/Early_Warning_Signal/' #'C:/Users/kdrake/Documents/GitHub/Early_Warning_Signal/'
filename <- "data_2022-May-09 - UK Covid-19 cases by sample date.csv" 
dat_type <- "cases" 
cases_df <- data_load( folder , filename , dat_type ) # Outputs cases_df(date,cases,time,wday)
...
#' Replace for loop method below with lapply method
...
for (gsf in gsf_list){
  for (k in k_list) {
    for (gt in gt_list) {
      params = data.frame( gsf , k , gt )
      params_df = rbind( params_df , params )
      counter = counter + 1
      params_list[[ counter ]] = params
}}}
   
wave_define_df = lapply( seq_along( params_list ) , function( x , i ){ 
  #message( "gsf = ",params_list[[ i ]][1], ", deg_free_k = ", params_list[[ i ]][ 2 ] ," gt = ", params_list[[ i ]][ 3 ] )
                                                  gsf = params_list[[ i ]][1] 
                                                  deg_free_k = params_list[[ i ]][ 2 ] 
                                                  gt = params_list[[ i ]][ 3 ] 
                                                  m <- gam_fitting(cases_df = cases_df, GAM_smooth_function = gsf, deg_free_k = k)
...
#' for loop version of lapply version above
counter = 0
gsf_list = c("tp","ts","ds","ps","cp","re","gp","cr","cs","cc") #,"mrf")
k_list = seq(50,300,10)
gt_list = seq(0,0.1,0.01)
...
write.csv(wave_define_df, file="wave_define_cases_growth-remove-low-resolution_2023-06-26.csv")
##############################
