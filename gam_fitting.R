...
#' @param def_free_k Degrees of freedom used for time element in GAM model fit.
...
gam_fitting <- function( cases_df, GAM_smooth_function = "cc", deg_free_k = 30 ) 
{
  ...
  m <- mgcv::gam( cases ~ s( time, bs=GAM_smooth_function, k = deg_free_k ) + s( wday, bs=GAM_smooth_function, k = 7), family = quasipoisson, data = cases_df )
