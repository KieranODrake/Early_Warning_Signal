...
growth_method <- function( m, cases_df, dat_type, wave_bands_df, GAM_smooth_function, growth_threshold = 0, deg_free_k)
{
  ...
  for (i in 1:200){ #increased to 200 from 100
    j = i
  ...
  for (r in 1:200){#increased to 200 from 100
    s = r
  ...
  for (v in 1:length(wave_start_dates_adj)){
      rv[v+16+200] = as.Date(wave_start_dates_adj[v],origin = "1970-01-01") #increased to 200 from 100
  }  

  # NOT CURRENTLY ADJUSTING FOR WAVE RESOLUTION
  for (w in 1:length(wave_reset_dates)){
    rv[w+16+200+200] = as.Date(wave_reset_dates[v],origin = "1970-01-01") #increased to 200 from 100
  }
